defmodule Rushing.RepositoryTest do
  ...
  describe "get_pages/3" do
    ...
    test "with rigth params but no filter", %{inserted_data: inserted_data} do
      %{result: first_page_with_10} = Repository.get_pages(1, 10, nil, nil, nil)
      %{result: second_page_with_6} = Repository.get_pages(2, 10, nil, nil, nil)
    ...
    test "with rigth params with name filter", %{inserted_data: inserted_data} do
      %{result: result} = Repository.get_pages(1, 10, "13", nil, nil)
      [player_13] = result
    ...
    test "with rigth params with order_by filter sort_total_rushing_yards", %{
      ...
      %{result: result} = Repository.get_pages(1, 13, nil, "total_rushing_yards", "asc")
